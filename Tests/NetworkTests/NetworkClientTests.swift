...
final class HTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_performsGetRequestWithURL() {
        ...
        makeSUT().request(with: makeAPIRequest()) { _ in }
        
        wait(for: [exp], timeout: 1.0)
    }
    
    func test_getFromURL_failOnRequestError() {
        let requestError = NSError(domain: "any error", code: 1, userInfo: [:])
    ...
    func test_getFromURL_successOnHTTPURLResponseWithData() {
        ...
        XCTAssertEqual(resultValues?.response.url?.absoluteString, response.url?.absoluteString)
    ...
    private func makeAPIRequest() -> APIRequest {
        let apiRequest = APIRequestSpy(anyUrl: anyURL)
        return apiRequest
    }
    ...
    private func resultFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #file, line: UInt = #line) -> HTTPClientResult {
        ...
        sut.request(with: makeAPIRequest()) { result in
    ...
    private struct APIRequestSpy: APIRequest {
    ...
    private var anyURL: URL {
        return URL(string: "httpL//any-url.com")!
    }
    ...
    private class URLProtocolStub: URLProtocol {
        ...
        override func startLoading() {
            if URLProtocolStub.requestObserver != nil {
